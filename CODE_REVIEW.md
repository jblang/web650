...
A web-based UI for the Open SIMH IBM 650 simulator, a historic computer from the mid-1950s. Built with Next.js 16.1.4, React 19.2.3, TypeScript (strict mode), and IBM Carbon Design System. The simulator runs entirely in the browser via WebAssembly, compiled from Open SIMH using Emscripten. A modular TypeScript wrapper (`src/lib/simh/`) provides the FFI layer over the Emscripten module, with generic SIMH framework code separated from I650-specific functionality in `src/lib/simh/i650/`. `EmulatorProvider.tsx` manages all emulator state via React context.
...
**Note:** The React Compiler is enabled (`next.config.ts` line 4), which may auto-memoize some of these. However, the compiler cannot fix architectural issues.
...
| `src/lib/simh/core.ts` | **HIGH** — Core WASM integration including module loading, output parsing, and capture logic |
| `src/lib/simh/i650/*.ts` | **MEDIUM** — I650-specific register access, memory operations, and validation |

**`src/lib/simh/` — testability analysis:**
...
The wrapper code is now organized into generic SIMH framework (`src/lib/simh/`) and I650-specific modules (`src/lib/simh/i650/`):
...
1. Add unit tests for `parseKeyValues` in `core.ts` (parsing edge cases: empty input, malformed lines, mixed case)
2. Add unit tests for `postProcessI650Values` in `i650/memory.ts` (multi-digit AR truncation, edge cases)
3. Add unit tests for I650 validation functions in `i650/memory.ts` (`validateWord`, `validateAddress`, `extractOperationCode`)
4. Add integration tests that load the WASM module directly in Node.js via `require('./public/i650.js')` to test command execution, register examine/deposit, stepping, and filesystem operations
5. Add `EmulatorProvider` tests with `@/lib/simh` mocked via `vi.mock` (register refresh, display switch derivation, program start/stop/reset flows, manual drum read-out/write-in)
6. Consider refactoring `core.ts` `init()` to accept an injected module instance, enabling the wrapper's integration logic (output capture, command execution) to be tested against the real WASM module in Node.js
...
4. **React Compiler enabled.** The React Compiler provides automatic memoization, partially mitigating the missing manual `React.memo` calls.

5. **Appropriate use of Next.js features.** App Router, `'use client'` directives where needed, static page generation for all routes.

---

## Files to Prioritize for Improvement

1. **`src/lib/simh/core.ts`** + **`src/lib/simh/i650/memory.ts`** — Add unit tests for `parseKeyValues`, `postProcessI650Values`, and validation functions
2. **`src/components/EmulatorProvider.tsx`** — Add tests with mocked `@/lib/simh`
